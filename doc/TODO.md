...
| **최종 갱신** | 2026-05-20 |
| **현재 단계** | TDD **RED (01)** 완료 → **GREEN (01)** 대기 |
| **원격** | `origin/red` @ `b30c01c` ([UnitConverter_10](https://github.com/engmater6-dev/UnitConverter_10)) |

---

## 📍 현재 진행 스냅샷 (2026-05-20)

### TDD 사이클

| 단계 | 상태 | 증거 |
|------|------|------|
| **RED** | ✅ 완료 | `tests/entity/` 2파일 · `tests/boundary/` 3파일 · [report/01.red.md](../report/01.red.md) |
| **GREEN** | 🔲 대기 | [task/green/01.green.md](../task/green/01.green.md) 프롬프트 준비 |
| **REFACTOR** | 🔲 — | GREEN 이후 |

### pytest (RED 검증)

```text
py -3 -m pytest tests/ -v  →  exit 4, ModuleNotFoundError: No module named 'entity'
```

- ImportError로 **수집 TC 0건** — assert 단계 미도달 (의도적 RED)
- GREEN 후 동일 명령으로 **exit 0 · assert pass** 재검증 필요

### RED 테스트 ↔ Must 매핑

| ID | RED TC | 구현(GREEN) | 비고 |
|----|--------|-------------|------|
| M-01 | ✅ `test_registry.py` | 🔲 | |
| M-02 | ✅ `test_conversion.py` | 🔲 | |
| M-04 | 🟡 `test_input_parser.py` (5종) | 🔲 | `INVALID_UNIT_ID` TC 미포함 |
| M-05 | 🟡 `meter:-1` only | 🔲 | `NON_FINITE_VALUE` TC 미포함 |
| M-06 | ✅ `test_output_formatter.py` | 🔲 | CLI·진입점 없음 |
| M-07 | ✅ `test_error_presenter.py` (5종) | 🔲 | |
| M-09 | 🟡 RED fail 확인 | 🔲 | `tests/entity/` 0 fail 미달성 |

---

## 🔴 필수 (Must-Have) — v1.0 릴리스 차단 항목

- [ ] **M-01** entity 레이어 골격 및 Registry 3단위 고정 | PRD F-03, §5.1, §3.3 Background | 🟡 **RED** `tests/entity/test_registry.py` · **GREEN** `entity/unit_registry.py` 🔲
- [ ] **M-02** ConversionEngine `convert_all` meter 경유 환산 | PRD F-01, F-03, G-01, AC-01 | 🟡 **RED** `tests/entity/test_conversion.py` · **GREEN** `entity/conversion_engine.py` 🔲
- [ ] **M-03** feet↔yard 직접 비율 상수 0건 | PRD F-03, G-05, REG-01 | **리뷰어**가 entity/control/boundary 정적 검색·TC에서 직접 상수 0건 확인 후 pass
- [ ] **M-04** CONVERT 파싱·검증 (F-02 전 code) | PRD F-02, US-01, AC-02~04 | 🟡 **RED** 5종 (`test_input_parser.py`) · `INVALID_UNIT_ID` TC 🔲 · **GREEN** `boundary/cli_input_parser.py` 🔲
- [ ] **M-05** NEG-01 음수·non-finite 거부 | PRD §3.2 NEG-01, AC-03 | 🟡 **RED** `meter:-1` only · NaN/Inf TC 🔲 · **GREEN** 🔲
- [ ] **M-06** table 출력·표현 계약 (LHS 입력 보존) | PRD F-04, §6.1, AC-01, AC-05 | 🟡 **RED** `test_output_formatter.py` · CLI 실행·리뷰어 캡처 🔲
- [ ] **M-07** ErrorPresenter stderr code·message·exit | PRD F-05, G-02 | 🟡 **RED** 5종 (`test_error_presenter.py`) · **GREEN** `boundary/error_presenter.py` 🔲
- [ ] **M-08** boundary/control/entity/data 디렉터리·의존 방향 | PRD §4.2, F-11, REG-06 | 🟡 `tests/`·`main/` 존재 · `entity/`·`boundary/`·`control/`·`data/` 🔲
- [ ] **M-09** Domain RED→GREEN 게이트 (entity I/O 금지) | PRD F-11, US-07 | 🟡 **RED** ImportError 확인 ([report/01.red.md](../report/01.red.md)) · **GREEN** `tests/entity/` 0 fail 🔲
...
- [ ] **S-11** README 실행·계약 요약 갱신 | PRD 부록 C, Phase 6 이관 | 🟡 [README.md](../README.md) 진행 상황·pytest·디렉터리 갱신 · 목표 CLI·cov 명령 🔲
...
- [ ] **N-06** `.cursorrules` / `pyproject.toml` 프로젝트 규칙 고정 | PRD §4.1 | 🟡 `.cursorrules` 존재 · `pyproject.toml` 🔲

---

## 🔵 기술 부채 (Tech Debt)

- [ ] **TD-01** 단일 파일 `UnitConverter.py` if-elif·하드코딩 비율 | PRD §1.2 | 🟡 `main/UnitConverter.py`로 이동 · BCE 대체·deprecate 🔲
...
- [ ] **TD-04** README 예시만 있고 PRD 계약(NEG-01·표현·EPS) 미문서화 | PRD §7, Phase 6 | 🟡 README 진행 상황·에러 code·EPS 반영 · `doc/CONTRACT.md` 분리 🔲
...
- [ ] **TD-07** 테스트·src 레이어 디렉터리 부재 | PRD §4.2 | 🟡 `tests/`·`pytest.ini`·`.gitignore` · `entity/`·`boundary/` src 🔲
...
- [x] 레거시 `UnitConverter.py` 기본 변환 프로토타입 (meter/feet/yard) | (선행) | `main/UnitConverter.py` — **PRD 인수 미달**
- [x] **RED** Domain·Boundary pytest 계약 고정 | 2026-05-20 | `tests/entity/` 2 · `tests/boundary/` 3 · `conftest`·`helpers`·`pytest.ini`
- [x] RED 검증·기록 | 2026-05-20 | [prompt/01.red.md](../prompt/01.red.md) · [report/01.red.md](../report/01.red.md) · pytest ImportError 확인
- [x] GREEN 작업 프롬프트 | 2026-05-20 | [task/green/01.green.md](../task/green/01.green.md)
- [x] README 진행 상황 섹션 | 2026-05-20 | TDD RED·디렉터리·pytest·TODO 연동
- [x] GitHub `red` 브랜치 push | 2026-05-20 | `b30c01c` test: add RED pytest suite and TDD scaffolding
...
| **M1 Domain GREEN** | F-03, F-01(entity), F-11(entity), M-01~M-05, M-09, M-14, G-01 | T+1일 | 🟡 **RED Done** · GREEN 🔲 | [task/green/01.green.md](../task/green/01.green.md) |
...
**다음 작업:** [task/green/01.green.md](../task/green/01.green.md) 실행 → `entity/`·`boundary/` 최소 구현 → `py -3 -m pytest tests/ -v` **exit 0**
